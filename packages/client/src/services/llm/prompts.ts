 ...
 * LLM 提示词模块（浏览器侧）
 *
 * 从原 server 端 services/llm/prompts.ts 迁移而来：图→文本、系统提示词、四类分析提示词。
 * 结构化结果类型沿用 types/index.ts 中已有定义，这里不重复声明。
 */
...
export function convertGraphToText(
  ...
  if (nodes.length === 0) {
    ...
    nodes.forEach((node) => {
      ...
      const statusKey = (node.baseStatus as string) || '';
      const statusName = STATUS_MAP[statusKey] || statusKey;
      text += `- ID: ${node.id}\n`;
      text += `  类型: ${typeName}\n`;
      text += `  名称: ${node.title}\n`;
      if (statusName) text += `  状态: ${statusName}\n`;
      if (node.content) {
 ...
 * 风险分析提示词 - 结构化输出
 */
const RISK_ANALYSIS_PROMPT = `分析计划风险，返回结构化数据。
 ...
 * 下一步建议提示词 - 结构化输出
 */
const NEXT_STEP_PROMPT = `分析当前状态，建议下一步行动顺序。
 ...
 * 逻辑检查提示词 - 结构化输出
 */
const LOGIC_CHECK_PROMPT = `检查规划图的逻辑完整性。
 ...
 * 补全建议提示词 - 结构化输出
 */
const COMPLETION_PROMPT = `分析当前图结构，建议可能遗漏的节点和关系。
...
export const ERROR_MESSAGES = {
  API_KEY_MISSING: '未配置 API Key，请点击右上角「设置」填写',
